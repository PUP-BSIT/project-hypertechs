...
function showCustomerData(data) {
  ...
  accAddress = document.querySelector(ID_ADDRESS);

  if (accAddress) {
    accAddress.innerHTML = data.data.accAddress;
  }
  if (firstName) {
  ...
  if (accountName || accountNumber || balance || lastTransac) {
    ...
    totalTransferred.innerHTML = strToNum(data.data.totalTransferred);
    totalReceived.innerHTML = strToNum(data.data.totalReceived);
    averageTransferred.innerHTML = strToNum(data.data.averageTransferred);
    ...
    const partialDisplayElementAccount = document.querySelector(
      "#display_account_number"
    );
    const partialAccountNumber =
      data.data.accountNumber.substring(0, 4) + "••••••••";
    partialDisplayElementAccount.innerHTML = partialAccountNumber;

    const partialDisplayElementCard = document.querySelector(
      "#display_card_number"
    );
...
function toggleAccountNumberDisplay() {
  ...
  if (accountNumberElement && partialDisplayElement) {
    ...
    const maskedPart = fullAccountNumber.substring(4).replace(/./g, "•");
